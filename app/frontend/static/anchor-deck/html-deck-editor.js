  ...
  function normalizePresentation(input) {
    ...
    class DeckEditor {
      constructor(presentation) {
        ...
        this.genericImportMode = Boolean(this.stage?.ownerDocument?.body?.hasAttribute("data-html-deck-editor-generic"));
        ...
        this.ensurePanelToggleControls();
        this.bindControls();
      ...
      ensurePanelToggleControls() {
        if (!this.shell || this.panelToggles) return;
        const slides = this.shell.querySelector(".editor-slides");
        const panel = this.shell.querySelector(".editor-panel");
        if (!slides || !panel) return;
        const makeToggle = (side, label, target) => {
          if (!target.id) target.id = side === "left" ? "editor-slides" : "editor-panel";
          const button = document.createElement("button");
          button.type = "button";
          button.className = `editor-panel-toggle editor-panel-toggle-${side}`;
          button.dataset.htmlDeckEditorUi = "";
          button.dataset.panelSide = side;
          button.setAttribute("aria-controls", target.id || (side === "left" ? "editor-slides" : "editor-panel"));
          button.setAttribute("aria-expanded", "true");
          button.title = label;
          button.textContent = side === "left" ? "<" : ">";
          button.addEventListener("click", () => this.togglePanel(side));
          this.shell.appendChild(button);
          return button;
        };
        this.panelToggles = {
          left: makeToggle("left", "隐藏幻灯片栏", slides),
          right: makeToggle("right", "隐藏属性栏", panel),
        };
      }

      togglePanel(side, force) {
        if (side !== "left" && side !== "right") return;
        const className = side === "left" ? "editor-slides-collapsed" : "editor-panel-collapsed";
        const collapsed = typeof force === "boolean" ? force : !document.body.classList.contains(className);
        document.body.classList.toggle(className, collapsed);
        const button = this.panelToggles?.[side];
        if (button) {
          button.setAttribute("aria-expanded", String(!collapsed));
          button.title = collapsed ? (side === "left" ? "展开幻灯片栏" : "展开属性栏") : (side === "left" ? "隐藏幻灯片栏" : "隐藏属性栏");
      ...
      prepareEditableElements() {
        ...
        this.markGenericImportElements();
        });
      }

      // Imported HTML often has no generator-specific data attributes. Keep
      // generic discovery opt-in so PPT/AI decks retain their existing
      // selection semantics, while imported documents remain editable.
      markGenericImportElements() {
        if (!this.genericImportMode) return;
        const textTags = "h1,h2,h3,h4,h5,h6,p,li,blockquote,figcaption,caption,td,th,button,a,label,pre,dt,dd";
        const mediaTags = "img,picture,video,canvas,svg,iframe,object,embed";
        stageSlides(this.stage).forEach((slide) => {
          slide.querySelectorAll(mediaTags).forEach((element, index) => {
            if (this.shouldIgnoreEditorCandidate(element) || element.matches("defs *")) return;
            if (!element.dataset.editorKind) this.markEditorKind(element, "media", true);
            if (!element.dataset.editId) element.dataset.editId = `import-media-${index}`;
          });
          slide.querySelectorAll(textTags).forEach((element, index) => {
            if (this.shouldIgnoreEditorCandidate(element) || element.querySelector(mediaTags)) return;
            if (!this.hasVisibleText(element)) return;
            if (this.hasNestedTextContainerChild(element)) return;
            const rect = this.elementClientRect(element);
            if (rect.width < 2 || rect.height < 2) return;
            if (!element.dataset.editorKind) this.markEditorKind(element, "text", true);
            if (!element.dataset.editId) element.dataset.editId = `import-text-${index}`;
          });
          slide.querySelectorAll("div,span,section,article").forEach((element, index) => {
            if (this.shouldIgnoreEditorCandidate(element) || element.querySelector(`${textTags},${mediaTags}`)) return;
            if (!this.hasVisibleText(element) || !this.hasDirectText(element)) return;
            const rect = this.elementClientRect(element);
            if (rect.width < 2 || rect.height < 2) return;
            if (!element.dataset.editorKind) this.markEditorKind(element, "text", true);
            if (!element.dataset.editId) element.dataset.editId = `import-inline-${index}`;
          });
          slide.querySelectorAll("*").forEach((element, index) => {
            if (this.shouldIgnoreEditorCandidate(element) || element.dataset.editorKind) return;
            const style = getComputedStyle(element);
            if (style.backgroundImage === "none") return;
            const rect = this.elementClientRect(element);
            if (rect.width < 32 || rect.height < 24 || rect.width * rect.height < 1600) return;
            this.markEditorKind(element, "media", true);
            if (!element.dataset.editId) element.dataset.editId = `import-background-${index}`;
          });
        });
      }
      ...
      isDraggableEditable(element) {
        return Boolean(
          ...
          (element.classList.contains("editor-layer") ||
            element.matches("[data-editable], [data-editable-media], [data-editable-box], [data-editor-kind='text'], [data-editor-kind='media'], [data-editor-kind='box']"))
        );
      }
      ...
      editorInsets() {
        ...
        if (document.body?.hasAttribute("data-ppt-workbench-contained-editor") && window.innerWidth > 960) {
          const leftCollapsed = document.body.classList.contains("editor-slides-collapsed");
          const rightCollapsed = document.body.classList.contains("editor-panel-collapsed");
          insets.left = Math.max(insets.left, leftCollapsed ? 24 : 340);
          insets.right = Math.max(insets.right, rightCollapsed ? 24 : 470);
          insets.top = Math.max(insets.top, 96);
          insets.bottom = Math.max(insets.bottom, 24);
        }
      ...
      toggleEditMode(force) {
        ...
        if (!this.isActive) {
          document.body.classList.remove("editor-slides-collapsed", "editor-panel-collapsed");
      ...
      getEditableTarget(target) {
        ...
        if (!element && this.genericImportMode) element = this.getGenericImportTarget(target);
        if (!element || !this.stage.contains(element)) return null;
      ...
      getGenericImportTarget(target) {
        if (!this.genericImportMode || !target) return null;
        let node = target.nodeType === Node.ELEMENT_NODE ? target : target.parentElement;
        const active = this.activeSlide();
        if (!node || !active || !active.contains(node)) return null;
        if (this.isEditorUiElement(node) || node.closest("script,style,template,defs")) return null;
        const media = node.closest("img,picture,video,canvas,svg,iframe,object,embed");
        if (media && active.contains(media)) return media;
        const text = node.closest("h1,h2,h3,h4,h5,h6,p,li,blockquote,figcaption,caption,td,th,button,a,label,pre,dt,dd");
        if (text && active.contains(text) && this.hasVisibleText(text)) return text;
        while (node && node !== active) {
          if (this.hasVisibleText(node) && this.hasDirectText(node)) {
            const rect = this.elementClientRect(node);
            if (rect.width >= 2 && rect.height >= 2) return node;
          }
          const style = getComputedStyle(node);
          if (style.backgroundImage && style.backgroundImage !== "none") {
            const rect = this.elementClientRect(node);
            if (rect.width >= 32 && rect.height >= 24) return node;
          }
          node = node.parentElement;
        }
        return null;
